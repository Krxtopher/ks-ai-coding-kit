...
- **Install targets** can use two modes:
  ...
  - **append** — object target, e.g. `claude-code: { file: CLAUDE.md, mode: append }`. The source content is appended to the target file, wrapped in HTML comment markers (`<!-- ks-ai-coding-kit:append:<name> -->`) for clean uninstall.
